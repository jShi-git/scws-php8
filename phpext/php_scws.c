...
#define	SCWS_FETCH_PARAMETERS(ts, ...)	\
	do {	\
		...
		if (obj) {	\
			/*	\
			 * Use zend_read_property so it works for declared properties too.	\
			 * (PHP 8.2 deprecates dynamic properties; scws.stub.php declares $handle.)	\
			 */	\
			handle_zv = zend_read_property(Z_OBJCE_P(obj), Z_OBJ_P(obj), "handle", sizeof("handle") - 1, 1, NULL);	\
			if (handle_zv == NULL || Z_TYPE_P(handle_zv) != IS_RESOURCE) {	\
				php_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid or missing SimpleCWS::handle");	\
				RETURN_FALSE;	\
			}	\
			...
			/* Function style call: first argument is resource handle */	\
			if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "r" ts, &handle_zv, ##__VA_ARGS__) == FAILURE) {	\
				return;	\
			}	\
		}	\
		ps = (struct php_scws *) zend_fetch_resource_ex(handle_zv, PHP_SCWS_OBJECT_TAG, le_scws);	\
	} while(0)
#else
#define	SCWS_FETCH_PARAMETERS(ts, ...)	\
	do {	\
...
#if PHP_MAJOR_VERSION >= 8
	/* Register class entries generated from scws.stub.php (declares $handle) */
...
PHP_FUNCTION(scws_new)
{
	...
	/// jShi-git.260202: set SimpleCWS::handle via zend_update_property
	{
		...
		zend_update_property(Z_OBJCE_P(return_value), Z_OBJ_P(return_value), "handle", sizeof("handle") - 1, &handle_zv);
	}
#else
